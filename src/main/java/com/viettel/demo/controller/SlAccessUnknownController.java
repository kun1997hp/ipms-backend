...
import com.viettel.demo.service.SlAccessUnknownService;
...
@RestController
@RequestMapping("/sl-access-unknown")
public class SlAccessUnknownController {
    @Autowired
    private SlAccessUnknownService slAccessUnknownService;

    @Autowired
    private SuccessMessage successMessage;

    @GetMapping("/v1")
    public ResponseEntity<DataTableResponse> getSlAccessUnknownV1(
            @And({
                    ...
                    //date
                    //date
            }) Specification<SlAccessUnknown> specs, Pageable pageable) {
        DataTable dataTable = slAccessUnknownService.findAllPagingAndSorting(specs, pageable);
