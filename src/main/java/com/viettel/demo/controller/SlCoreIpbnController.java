...
import com.viettel.demo.service.SlCoreIpbnService;
...
@RestController
@RequestMapping("/sl-core-ipbn")
public class SlCoreIpbnController {
    @Autowired
    private SlCoreIpbnService slCoreIpbnService;

    @Autowired
    private SuccessMessage successMessage;

    @GetMapping("/v1")
    public ResponseEntity<DataTableResponse> getSlCoreIpbnV1(
            @And({
                    ...
                    //date
                    //date
            }) Specification<SlCoreIpbn> specs, Pageable pageable) {
        DataTable dataTable = slCoreIpbnService.findAllPagingAndSorting(specs, pageable);
