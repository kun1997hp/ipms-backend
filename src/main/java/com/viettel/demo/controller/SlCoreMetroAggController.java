...
import com.viettel.demo.service.SlCoreMetroAggService;
...
@RestController
@RequestMapping("/sl-core-metro-agg")
public class SlCoreMetroAggController {
    @Autowired
    private SlCoreMetroAggService slCoreMetroAggService;

    @Autowired
    private SuccessMessage successMessage;

    @GetMapping("/v1")
    public ResponseEntity<DataTableResponse> getSlCoreMetroAggV1(
            @And({
                    ...
                    //date
                    //date
            }) Specification<SlCoreMetroAgg> specs, Pageable pageable) {
        DataTable dataTable = slCoreMetroAggService.findAllPagingAndSorting(specs, pageable);
