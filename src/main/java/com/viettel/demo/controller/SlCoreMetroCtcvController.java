...
import com.viettel.demo.service.SlCoreMetroCtcvService;
...
@RestController
@RequestMapping("/sl-core-metro-ctcv")
public class SlCoreMetroCtcvController {
    @Autowired
    private SlCoreMetroCtcvService slCoreMetroCtcvService;

    @Autowired
    private SuccessMessage successMessage;

    @GetMapping("/v1")
    public ResponseEntity<DataTableResponse> getSlCoreMetroCtcvV1(
            @And({
                    ...
                    //date
                    //date
            }) Specification<SlCoreMetroCtcv> specs, Pageable pageable) {
        DataTable dataTable = slCoreMetroCtcvService.findAllPagingAndSorting(specs, pageable);
