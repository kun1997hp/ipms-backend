...
import com.viettel.demo.service.SlCoreMpbnService;
...
@RestController
@RequestMapping("/sl-core-mpbn")
public class SlCoreMpbnController {
    @Autowired
    private SlCoreMpbnService slCoreMpbnService;

    @Autowired
    private SuccessMessage successMessage;

    @GetMapping("/v1")
    public ResponseEntity<DataTableResponse> getSlCoreMpbnV1(
            @And({
                    ...
                    //date
                    //date
            }) Specification<SlCoreMpbn> specs, Pageable pageable) {
        DataTable dataTable = slCoreMpbnService.findAllPagingAndSorting(specs, pageable);
