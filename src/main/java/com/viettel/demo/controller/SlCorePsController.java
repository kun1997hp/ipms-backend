...
import com.viettel.demo.service.SlCorePsService;
...
@RestController
@RequestMapping("/sl-core-ps")
public class SlCorePsController {
    @Autowired
    private SlCorePsService slCorePsService;

    @Autowired
    private SuccessMessage successMessage;

    @GetMapping("/v1")
    public ResponseEntity<DataTableResponse> getSlCorePsV1(
            @And({
                    ...
                    //date
                    //date
            }) Specification<SlCorePs> specs, Pageable pageable) {
        DataTable dataTable = slCorePsService.findAllPagingAndSorting(specs, pageable);
